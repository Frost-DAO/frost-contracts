...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SFROST }

    address public immutable FROST;
    ...
    address public sFROST;
    uint public sFROSTQueue; // Delays change to sFROST address
    ...
    if ( _FROSTDAI != address(0) ) {
        isLiquidityToken[ _FROSTDAI ] = true;
        liquidityTokens.push( _FROSTDAI );
    }
    ...
    /**
        @notice allow approved address to deposit an asset for FROST
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint FROST needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( FROST ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn FROST for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        IFROSTERC20( FROST ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sFROST ).balanceOf( msg.sender ); // Can only borrow against sFROST held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with FROST
        @param _amount uint
     */
    function repayDebtWithFROST( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IFROSTERC20( FROST ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, FROST, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( FROST ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( FROST ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns FROST valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match FROST decimals
            value_ = _amount.mul( 10 ** IERC20( FROST ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SFROST ) { // 9
            sFROSTQueue = block.number.add( blocksNeededForQueue );
        } else return false;
